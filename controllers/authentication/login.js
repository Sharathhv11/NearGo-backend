...
const login = handelAsyncFunction(async (req, res, next) => {
  ...
  if (!user || !user.verified) {
    return next(
      new CustomError(
        401,
        `No user is registerd with e-mail ${email}. Please create the account.`
      )
    );
  }

  //^ verify weather account created using the google oauth2.0
  ...
  if (!result) return next(new CustomError(401, "Invalid password!."));
