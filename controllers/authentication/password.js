...
const forgotPassword = handelAsyncFunction(async (req, res, next) => {
  ...
  if (!email) return next(new CustomError(400, "Email is required feild."));
  ...
  if (!user || !user.verified)
    return next(
      new CustomError(
        400,
        `No user exists with email ${email}. Please try again.`
      )
    );

  //^ if user exits  and used the google oauth for account creation we will stop him from resetting the password
